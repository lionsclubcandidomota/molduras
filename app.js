(() => {
  ...
  function normalizeData() {
    ...
    state.categories = rawCategories
      ...
      .map((c, i) => ({
        id: String(c.id || slug(c.nome) || `categoria-${i + 1}`),
        nome: String(c.nome || 'Outras'),
        ordem: Number(c.ordem) || i + 1
      }))
      .sort((a, b) => a.ordem - b.ordem);

    state.frames = rawFrames
      .filter(f => f && f.ativo !== false && f.id && f.nome && (f.arquivo || f.imagem || f.src))
      .map((f, i) => ({
        ...f,
    ...
    const categoryByName = new Map(state.categories.map(c => [c.nome.trim().toLocaleLowerCase('pt-BR'), c]));

    state.frames.forEach((frame, index) => {
      let category = frame.categoriaId ? categoryById.get(frame.categoriaId) : null;
      const legacyName = String(frame.categoriaNome || frame.categoria || '').trim();

      if (!category && legacyName) {
        category = categoryByName.get(legacyName.toLocaleLowerCase('pt-BR')) || null;
      }

      if (!category) {
        const nome = legacyName || 'Outras';
        let id = slug(nome) || `categoria-${state.categories.length + 1}`;
        let suffix = 2;
        while (categoryById.has(id)) id = `${slug(nome) || 'categoria'}-${suffix++}`;
        category = { id, nome, ordem: state.categories.length + 1 };
        state.categories.push(category);
        categoryById.set(id, category);
        categoryByName.set(nome.toLocaleLowerCase('pt-BR'), category);
      }

      frame.categoriaId = category.id;
      frame.ordem = Number(frame.ordem) || index + 1;
    });

    state.categories.sort((a, b) => a.ordem - b.ordem);
  }

(() => {
  ...
  function setEnabled(enabled) {
    [zoomRange, rotationRange, downloadBtn, shareBtn, fitBtn, centerBtn, resetBtn].forEach(el => el.disabled = !enabled);
    emptyState.hidden = enabled; adjustHint.hidden = !enabled;
    photoStatus.textContent = enabled ? 'Foto carregada ✓' : 'Aguardando foto'; photoStatus.classList.toggle('is-ready', enabled);
    photoButton.textContent = enabled ? '🔄 Trocar foto' : '📷 Escolher foto';
    byId('controls').setAttribute('aria-disabled', enabled ? 'false' : 'true');
    mobileActionBar.classList.toggle('is-visible', enabled); document.body.classList.toggle('has-mobile-bar', enabled);
